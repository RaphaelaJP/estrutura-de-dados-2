...
public class HashEncadeado<G> extends Hash<G> {
	...
	protected boolean finalizarInsercao(int indice, G conteudo) {
		...
		/**
		 * Verifico se é null, inicializo a lista e após, insiro o elemneto.
		 */
		if (this.getTabela()[indice] == null) {
			this.tabela[indice] = new ListaSimples<>();
		}
	...
	protected boolean finalizarRemocao(int indice, G conteudo) {
		...
		/**
		 * Quando o indice < 0; retorna false(-1) por ser uma remoção mal sucedida;
		 * Se não, pega o indice e remove o conteúdo contido no indice.
		 */
		if (indice < 0) {
			return false;
		}
